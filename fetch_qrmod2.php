...
try {
    $query = 'SELECT assy_code, model_name, kepi_lot, operator_name, shift, asmline, line, qty_input FROM mod1_process WHERE qr_code = :qr_code';
    ...
    if (!$row) {
        echo json_encode(['success' => false, 'message' => 'This QR Code does no exist in the system.', 'title' => 'QR not Found']);
        exit;
    }
    ...
    $stmt = $conn->prepare($query);
    $stmt->execute([':qr_code' => $qr_code, ':qr_code2' => $qr_code]);
