...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $checkNG = "SELECT COUNT(*) FROM vi_process WHERE qr_code = :qr_code AND serial_status IN ('NO GOOD','ONGOING REPAIR')";
        $stmtNG = $conn->prepare($checkNG);
        $stmtNG->execute([':qr_code' => $qr_code]);
        $ngCount = $stmtNG->fetchColumn();

        if ($ngCount > 0) {
            $response['message'] = "AI Process blocked. Some serials are still NG or under Repair.";
            echo json_encode($response);
            exit;
        }
        ...
        $finalQtyQuery = "SELECT COALESCE(SUM(TRY_CAST(qty_input AS INT)), 0) FROM ai_process 
                          WHERE kepi_lot = :kepi_lot AND line = :line";
        ...
        if ($serials) {
            $insertSerial = "INSERT INTO ai_process (qr_code, qty_input, final_qtyinput, operator_name, shift, line, assy_code, model_name, kepi_lot, serial_code, created_at, board_status, serial_status, prev_boardstatus, prev_serialstatus) 
