    ...
    <div class="output-container">
        <form method="POST" class="filter-form">
            ...
            <select name="shift" id="shift" required>
                ...
                <option value="day" <?= (isset($_POST['shift']) && $_POST['shift'] == 'day') ? 'selected' : '' ?>>Day Shift (6 AM - 6 PM)</option>
                <option value="night" <?= (isset($_POST['shift']) && $_POST['shift'] == 'night') ? 'selected' : '' ?>>Night Shift (6 PM - 6 AM)</option>
            </select>
        ...
        <?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['line'], $_POST['shift'], $_POST['date'])): ?>
            <form method="POST" action="download_hourlyoutput.php" style="margin-top: 10px;">
                <input type="hidden" name="line" value="<?= htmlspecialchars($_POST['line']) ?>">
                <input type="hidden" name="shift" value="<?= htmlspecialchars($_POST['shift']) ?>">
