...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$checkResult) {
            throw new Exception('Serial code does not exist in Batchlot Process.');
        }

        $stmt = $conn->prepare('SELECT TOP 1 board_counter FROM partside_batchlot WHERE kepi_lot = :kepi_lot AND line = :line ORDER BY id DESC');
        ...
        $stmt = $conn->prepare('SELECT COALESCE(SUM(CAST(qty_input AS INT)),0) FROM partside_batchlot WHERE kepi_lot = :kepi_lot AND line = :line');
        $stmt->execute([':kepi_lot' => $kepi_lot, ':line' => $line]);
        $final_qtyinput = (int) $stmt->fetchColumn() + $qty_input;

        $stmt = $conn->prepare("INSERT INTO partside_batchlot (qr_code, serial_code, qty_input, final_qtyinput, operator_name, shift, asmline, line, assy_code, model_name, kepi_lot, board_counter, created_at, board_status, serial_status, prev_boardstatus, prev_serialstatus) 
        ...
        $stmt = $conn->prepare('SELECT COUNT(*) FROM partside_batchlot WHERE kepi_lot = :kepi_lot AND line = :line');
        $stmt->execute([':kepi_lot' => $kepi_lot, ':line' => $line]);
