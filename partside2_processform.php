...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    try {
        ...
        $stmt = $conn->prepare("SELECT serial_status FROM partside2_process WHERE serial_code = :serial_code");
        $stmt->execute([':serial_code' => $serial_code]);
        $fvissStatus = $stmt->fetchColumn();

        if ($fvissStatus === 'NO GOOD') {
            throw new Exception('This serial is already tagged as NO GOOD and cannot be processed.');
        }

        $stmt = $conn->prepare("SELECT TOP 1 board_counter FROM partside2_process WHERE kepi_lot = :kepi_lot AND line = :line ORDER BY id DESC");
        ...
        $stmt = $conn->prepare("SELECT COALESCE(SUM(CAST(qty_input AS INT)),0) FROM partside2_process WHERE kepi_lot = :kepi_lot AND line = :line");
        $stmt->execute([':kepi_lot' => $kepi_lot, ':line' => $line]);
        $final_qtyinput = (int)$stmt->fetchColumn() + $qty_input;

        $stmt = $conn->prepare("INSERT INTO partside2_process (qr_code, serial_code, qty_input, final_qtyinput, operator_name, shift, asmline, line, assy_code, model_name, kepi_lot, board_counter, created_at, board_status, serial_status, prev_boardstatus, prev_serialstatus) 
