...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmtcheck = $conn->prepare('SELECT * FROM micro_batchlot WHERE serial_code = :serial_code');
        $stmtcheck->execute([':serial_code' => $serial_code]);
        $checkResult = $stmtcheck->fetch(PDO::FETCH_ASSOC);

        if (!$checkResult) {
            throw new Exception('Serial code does not exist in Micro Process.');
        }

        $stmtNG = $conn->prepare('SELECT serial_status FROM micro_batchlot WHERE serial_code = :serial_code');
        ...
        $stmt = $conn->prepare('SELECT TOP 1 board_counter FROM wi_batchlot WHERE kepi_lot = :kepi_lot AND line = :line ORDER BY id DESC');
        ...
        $stmt = $conn->prepare('SELECT COALESCE(SUM(CAST(qty_input AS INT)),0) FROM wi_batchlot WHERE kepi_lot = :kepi_lot AND line = :line');
        $stmt->execute([':kepi_lot' => $kepi_lot, ':line' => $line]);
        ...
        $stmt = $conn->prepare('SELECT COUNT(*) FROM wi_batchlot WHERE kepi_lot = :kepi_lot AND line = :line');
        $stmt->execute([':kepi_lot' => $kepi_lot, ':line' => $line]);
