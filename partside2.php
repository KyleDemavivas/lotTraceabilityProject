...
<head>
    ...
    <link rel="stylesheet" href="css/partside_process.css">
</head>
...
<body>
    <div class="form-container">
        <form method="POST" id="mainForm" name="mainForm">
            <h1>
                <center>Part Side 2</center>
            </h1>
            <div class="form-section">
                <div class="form-group">
                    <label class="form-label">Serial Code:</label>
                    <input type="text" class="form-input" name="serial_code" id="serial_code" autofocus autocomplete="off" minlength="13" maxlength="13" required>
                </div>
                <input type="text" class="form-input" name="qr_code" id="qr_code" autofocus autocomplete="off" minlength="21" maxlength="21" required hidden>
    ...
    <script>
        ...
        function checkAndAutoSubmit() {
            const serial_code = $('#serial_code').val();
            ...
            console.log(`qr: ${qr}`);
            console.log(`assy: ${assy}`);
            console.log(`model: ${model}`);
            console.log(`lot: ${lot}`);
            console.log(`asmline: ${asmline}`);
        ...
        $(document).ready(function() {
                    ...
                    serialDebounceTimer = setTimeout(() => {
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if (!(response.success) && (response.errorCode === 'serialnogood')) {
                                    Swal.fire({
                                        icon: 'warning',
                                ...
                                if (response.success) {
                                    ...
                                    $.ajax({
                                        url: 'get_boardcountPartside2.php',
                                        type: 'POST',
                                        dataType: 'json',
                                        data: {
                                            kepi_lot: kepi_lot,
                        ...
                        if (response.status === 'success') {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    $('input[name="final_qtyinput"]').val(response.board_count || 'ERROR');
                                    Swal.fire({
            ...
            $('#nogoodForm').submit(function(e) {
                ...
                $.ajax({
                    url: 'partside2_serialnogood.php',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            $('#modal_serial_code').on('input', function() {
                ...
                if (serial.length > 3) {
                    serialDebounceTimer = setTimeout(() => {
                        $.ajax({
                            url: 'partside2_validateserialmatch.php',
                            type: 'POST',
                            dataType: 'json',
                            data: {
                                serial_code: serial,
            ...
            $('#scrapButton').on('click', function(e) {
                ...
                getBoardData(serial_code, 'fetch_partside2.php', function(response) {
                    if (response.success === true) {

                    const scrapData = buildScrapData(qr_code, serial_code, response, location, defect, "PARTSIDE2", board_number);
