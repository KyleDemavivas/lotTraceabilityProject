    ...
    try {
        ...
        if ($row) {
            if ($row['mi_process'] === 'GOOD') {
                ...
                $response['message'] = 'MI PROCESS NOT FOUND';
                echo json_encode($response);
                exit;
            }
        }

        $query = "SELECT COUNT(process_location) FROM repair_master WHERE qr_code = :qr_code AND process_location = 'MOD1'";
        $stmt = $conn->prepare($query);
        $stmt->execute([':qr_code' => $qr_code]);
        $repaired = (int) $stmt->fetchColumn();

        if ($repaired > 0) {
            $finalQtyStmt = $conn->prepare('SELECT TOP 1 final_qtyinput FROM mod1_process WHERE kepi_lot = :kepi_lot ORDER BY created_at DESC');
