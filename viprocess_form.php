...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $checkQuery = 'SELECT vi_process FROM trace_process WHERE qr_code = :qr_code';
        ...
        if ($viProcess !== false) {
            if ($viProcess === 'GOOD') {
                $response['message'] = 'Visual Inspection Process already has data.';
                echo json_encode($response);
                exit;
            } else {
                $updateMounter = "UPDATE trace_process SET vi_process = 'GOOD' WHERE qr_code = :qr_code";
                $updateStmt = $conn->prepare($updateMounter);
                $updateStmt->execute([':qr_code' => $qr_code]);
            }
        }

        $query = "SELECT COUNT(process_location) FROM repair_master WHERE qr_code = :qr_code AND process_location = 'VI'";
        $stmt = $conn->prepare($query);
        $stmt->execute([':qr_code' => $qr_code]);
        $repaired = (int) $stmt->fetchColumn();

        if ($repaired > 0) {
            $finalQtyStmt = $conn->prepare('SELECT TOP 1 final_qtyinput FROM vi_process WHERE kepi_lot = :kepi_lot ORDER BY created_at DESC');
        ...
        if ($serials) {
            $insertSerial = 'INSERT INTO vi_process (qr_code, qty_input, final_qtyinput, operator_name, shift, line, assy_code, model_name, kepi_lot, serial_code, created_at, board_status, serial_status, prev_boardstatus, prev_serialstatus) 
            VALUES (:qr_code, :qty_input, :final_qtyinput, :operator_name, :shift, :line, :assy_code, :model_name, :kepi_lot, :serial_code, :created_at, :board_status, :serial_status, :prev_boardstatus, :prev_serialstatus)';
