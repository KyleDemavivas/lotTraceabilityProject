...
if (isset($_POST['qr_code'])) {
    ...
    $query = "SELECT assy_code, model_name, kepi_lot, operator_name, shift, line, qty_input
              FROM ai_process WHERE qr_code = :qr_code";
    try {
        $stmt = $conn->prepare($query);
        $stmt->execute([':qr_code' => $qr_code]);
        if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $finalQtyQuery = "SELECT final_qtyinput FROM mod1_process WHERE kepi_lot = :kepi_lot ORDER BY created_at DESC";
            ...
            $serialQuery = "SELECT serial_code1, serial_code2, serial_code3, serial_code4, serial_code5,
            ...
            echo json_encode(array_merge(
                ['success' => true, 'final_qtyinput' => $final_qtyinput, 'serial_qty' => $serial_qty],
                $row
            ));
        } else {
