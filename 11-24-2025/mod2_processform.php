...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $counterQuery = "SELECT board_counter FROM mod2_process WHERE kepi_lot = :kepi_lot AND line = :line ORDER BY id DESC";
        ...
        $checkQuery = "SELECT mod2_process FROM trace_process WHERE qr_code = :qr_code";
        $checkStmt = $conn->prepare($checkQuery);
        $checkStmt->execute([':qr_code' => $qr_code]);
        $viProcess = $checkStmt->fetchColumn();

        if ($viProcess !== false) {
            if ($viProcess === 'GOOD') {
                $response['message'] = 'Modificator 1 Process already has data.';
                echo json_encode($response);
                exit;
            } else {
                $updateMounter = "UPDATE trace_process SET mod2_process = 'GOOD' WHERE qr_code = :qr_code";
        ...
        $finalQtyQuery = "SELECT COALESCE(SUM(CAST(qty_input AS INT)),0) AS final_qty 
                          FROM mod2_process WHERE kepi_lot = :kepi_lot AND line = :line";
        ...
        if ($serials) {
            $insertSerial = "INSERT INTO mod2_process 
            (qr_code, qty_input, final_qtyinput, operator_name, shift, line, assy_code, model_name, kepi_lot, serial_code, board_counter, created_at, board_status, serial_status, prev_boardstatus, prev_serialstatus) 
            VALUES (:qr_code, :qty_input, :final_qtyinput, :operator_name, :shift, :line, :assy_code, :model_name, :kepi_lot, :serial_code, :board_counter, :created_at, :board_status, :serial_status, :prev_boardstatus, :prev_serialstatus)";
        ...
        $boardCountQuery = "SELECT COUNT(*) FROM mod2_process WHERE kepi_lot = :kepi_lot AND line = :line";
        ...
        $response['message'] = 'Modificator 2 Process recorded successfully.';
