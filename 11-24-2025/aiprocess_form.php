...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $finalQtyQuery = "SELECT COALESCE(SUM(TRY_CAST(qty_input AS INT)), 0) AS final_qty FROM ai_process WHERE kepi_lot = :kepi_lot AND line = :line";
        ...
        if ($serials) {
            $insertSerial = "INSERT INTO ai_process (qr_code, qty_input, final_qtyinput, operator_name, shift, line, assy_code, model_name, kepi_lot, serial_code, created_at, board_counter, board_status, serial_status, prev_boardstatus, prev_serialstatus) 
            VALUES (:qr_code, :qty_input, :final_qtyinput, :operator_name, :shift, :line, :assy_code, :model_name, :kepi_lot, :serial_code, :created_at, :board_counter, :board_status, :serial_status, :prev_boardstatus, :prev_serialstatus)";
