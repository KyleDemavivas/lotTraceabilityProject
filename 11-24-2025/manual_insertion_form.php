...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $finalQtyStmt = $conn->prepare("SELECT final_qtyinput FROM mi_process WHERE kepi_lot = :kepi_lot ORDER BY id DESC");
        $finalQtyStmt->execute([':kepi_lot' => $kepi_lot]);
        $previous_final_qty = $finalQtyStmt->fetchColumn();
        $previous_final_qty = $previous_final_qty ? (int)$previous_final_qty : 0;
        ...
        if ($serials) {
            $insertSerial = "INSERT INTO mi_process (qr_code, qty_input, final_qtyinput, operator_name, shift, line, assy_code, model_name, kepi_lot, serial_code, mi_reason, mi_remarks, created_at) 
                VALUES (:qr_code, :qty_input, :final_qtyinput, :operator_name, :shift, :line, :assy_code, :model_name, :kepi_lot, :serial_code, :mi_reason, :mi_remarks, :created_at)";
