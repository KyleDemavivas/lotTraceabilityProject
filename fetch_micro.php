...
try {
    ...
    $stmt = $conn->prepare("SELECT qr_code, assy_code, model_name, kepi_lot, operator_name, shift, asmline, line, qty_input FROM partside2_process WHERE TRIM(UPPER(serial_code)) = :serial_code");
    $stmt->execute([':serial_code' => $serial_code]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM partside2_process WHERE serial_code = :serial_code AND (serial_status = 'NO GOOD')");
    $stmt->execute([':serial_code' => $serial_code]);
