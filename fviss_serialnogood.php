...
if (
    isset($_POST['qr_code'], $_POST['serial_code'], $_POST['defect'], $_POST['location'], $_POST['board_number'], $_POST['scrap_fviss'], $_POST['repairable'], $_POST['action_fviss'])
) {
    try {
        ...
        $scrap_fviss = $_POST['scrap_fviss'];
        $repairable = $_POST['repairable'];
        $action_fviss = $_POST['action_fviss'];
        $source = isset($_POST['source']) ? $_POST['source'] : '';

        date_default_timezone_set('Asia/Manila');
        $created_at = date('Y-m-d H:i:s');

        $checkSerial = $conn->prepare("SELECT COUNT(*) FROM fviss_process WHERE serial_code = :serial_code");
        ...
        if ($source === 'alert' || $source === 'modal') {
            $verifySerialQR = $conn->prepare("SELECT COUNT(*) FROM fviss_process WHERE serial_code = :serial_code AND qr_code = :qr_code");
        ...
        if ($successfulInserts > 0) {
            $updateSerialStatusSql = "UPDATE fviss_process SET prev_serialstatus = serial_status, serial_status = 'NO GOOD' WHERE serial_code = :serial_code";
            $stmtUpdateStatus = $conn->prepare($updateSerialStatusSql);
            $stmtUpdateStatus->execute([':serial_code' => $serial_code]);

            $checkSql = "SELECT COUNT(*) FROM fviss_process WHERE qr_code = :qr_code AND serial_status = 'NO GOOD'";
            $stmtCheck = $conn->prepare($checkSql);
            $stmtCheck->execute([':qr_code' => $qr_code]);
            $noGoodCount = $stmtCheck->fetchColumn();

            if ($noGoodCount > 0) {
                $updateBoardStatusSql = "UPDATE fviss_process SET prev_boardstatus = board_status, board_status = 'HOLD' WHERE qr_code = :qr_code AND prev_boardstatus = 'GOOD' AND board_status = 'GOOD'";
