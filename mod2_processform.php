...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $checkQuery = "SELECT mod2_process, mod1_process FROM trace_process WHERE qr_code = :qr_code";
        //$checkQuery = "SELECT mod2_process, mi_process FROM trace_process WHERE qr_code = :qr_code";
        ...
        if ($row) {
            if ($row['mod1_process'] === 'GOOD') {
                if ($row['mod2_process'] === 'GOOD') {
                    $response['message'] = 'MOD 2 Process already has data.';
                    echo json_encode($response);
                    exit; 
                } else {
                ...
                $response['message'] = 'MOD 1 PROCESS NOT FOUND';
                echo json_encode($response);
                exit;
            }
        }
